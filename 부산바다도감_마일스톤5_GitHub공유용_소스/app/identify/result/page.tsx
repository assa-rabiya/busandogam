...
export default function IdentifyResultPage() {
  ...
  useEffect(() => { if (isReady && (!selectedImage || (!isAnalyzing && !result))) router.replace("/identify"); }, [isAnalyzing, isReady, result, router, selectedImage]);
  if (!isReady || !selectedImage) return <div className="page-loading">분석 데이터를 확인하는 중…</div>;

  const shootAgain = () => { clear(); router.push("/identify"); };
  ...
  return <ProtectedPage><AppShell><div className="result-label">✦ AI 분석 결과</div><section className="result-layout"><div className="result-photo"><SelectedImagePreview image={selectedImage} large /><div className="confidence-card"><span>AI 추정 신뢰도</span><strong>{result.confidence}%</strong><div className="progress"><i style={{ width: `${result.confidence}%` }} /></div></div></div><div className="result-info"><p className="eyebrow">MOST LIKELY SPECIES</p><h1>{result.koreanName}</h1><p className="scientific-name">{result.scientificName}</p><div className="result-tags"><span>{result.category}</span><span>{result.rarity}</span><span>신뢰도 {result.confidence}%</span></div><Link className="card-link" href={`/species/${result.id}`}>생물 상세 보기 →</Link><p className="result-summary">{result.summary}</p><h2>주요 특징</h2><ul className="feature-list">{result.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul></div></section>
    ...
    <div className="result-actions"><button className="primary-action" onClick={() => router.push("/discoveries/new")}>⌖ 발견 기록 저장하기</button><button className="outline-action" onClick={() => { setNotice("도감 등록을 위해 발견 정보를 입력해 주세요."); window.setTimeout(() => router.push("/discoveries/new"), 650); }}>▦ 내 도감에 추가하기</button><button className="outline-action" onClick={() => setShowCandidates((value) => !value)}>≋ 다른 후보 보기</button><button className="text-button" onClick={shootAgain}>↻ 다시 촬영하기</button></div>
