...
export default function Home() {
  ...
  return <AppShell>
    ...
    <section className="home-section"><div className="section-heading"><div><p className="eyebrow">TODAY&apos;S SPOTS</p><h2>오늘의 추천 탐험지</h2></div><Link className="text-link" href="/map">지도에서 보기 →</Link></div><div className="spot-grid">{locations.map((spot) => <article className="spot-card" key={spot.id}><ImageTile label={spot.imageLabel} alt={spot.name} /><div><span className={`difficulty ${spot.difficulty}`}>● {spot.difficulty}</span><h3>{spot.name}</h3><p>{spot.speciesHint}</p><dl><div><dt>추천 시간</dt><dd>{spot.bestTime}</dd></div><div><dt>환경</dt><dd>{spot.habitat}</dd></div></dl><Link href="/map" className="card-link">상세보기 →</Link></div></article>)}</div></section>
