...
export default function MapPage() {
  ...
  const resetMap = () => { if (mapFailed) { router.replace("/map"); return; } setCenter(mockMapAdapter.defaultCenter); setZoom(1); setSelectedGroupId(null); setSelectedRecordId(null); };
  ...
  return <ProtectedPage><AppShell><section className="map-page">
    ...
    <div className="map-workspace">
      ...
      <main className="map-results">
        ...
        {viewMode === "map" ? <div className="map-stage"><MapView groups={groups} center={center} zoom={zoom} selectedGroupId={selectedGroupId} currentLocation={currentLocation} locationStatus={locationStatus} onSelectGroup={selectGroup} onZoom={setZoom} onReset={resetMap} onRequestLocation={requestLocation} onMovePlace={movePlace} failed={mapFailed} />{selectedGroup && selectedRecord && <MapPopup group={selectedGroup} selected={selectedRecord} onPick={(record) => setSelectedRecordId(record.id)} onClose={() => { setSelectedGroupId(null); setSelectedRecordId(null); }} />}</div> : <MapListView records={filteredRecords} sort={sort} currentLocation={currentLocation} onSort={setSort} onSelect={selectListRecord} />}
