...
export default function IdentifyResultPage() {
  ...
  if (isAnalyzing) return <ProtectedPage><AppShell><section className="analysis-layout"><SelectedImagePreview image={selectedImage} large /><div className="analysis-progress"><p className="eyebrow">AI ANALYSIS</p><h1>생물의 특징을<br />살펴보고 있어요</h1><p>사진을 닫지 말고 잠시만 기다려 주세요.</p><div className="progress analysis-bar"><i style={{ width: `${progress}%` }} /></div><div className="progress-label"><b>{progress}%</b><span>예상 소요 시간 약 2초</span></div><ol>{stepInfo.map((step) => { const done = completedSteps.includes(step.id); const active = currentStep === step.id && !done; return <li key={step.id} className={done ? "done" : active ? "active" : ""}><b>{done ? "✓" : step.icon}</b><span>{step.label}</span><small>{done ? "완료" : active ? "분석 중" : "대기"}</small></li>; })}</ol></div></section></AppShell></ProtectedPage>;

  if (requiresRetake || !result) return <ProtectedPage><AppShell><section className="analysis-retry"><SelectedImagePreview image={selectedImage} large /><div><span className="retry-icon" aria-hidden="true">?</span><p className="eyebrow">NEED ANOTHER PHOTO</p><h1>생물을 확실히 확인하기 어려워요</h1><p>{error ?? "도감에 있는 생물과 충분히 일치하지 않습니다."}</p><ul><li>생물 전체가 화면 중앙에 보이게 촬영해 주세요.</li><li>흔들림과 역광을 피하고 무늬가 보이게 가까이 촬영해 주세요.</li><li>정체를 모르는 생물은 직접 만지지 마세요.</li></ul><button className="primary-action" onClick={shootAgain}>↻ 다른 사진 업로드하기</button></div></section></AppShell></ProtectedPage>;
