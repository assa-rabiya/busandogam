...
export default function DiscoveryCompleteClient() {
  const params = useParams<{ id: string }>();
  const searchParams = useSearchParams();
  const recordId = searchParams.get("id") ?? params.id;
  const { getDiscovery, records, isReady } = useDiscoveries();
  const record = getDiscovery(recordId);
  ...
  return <ProtectedPage><AppShell>
    ...
    <div className="complete-actions"><a className="primary-action" href={withBasePath(`/map?discoveryId=${record.id}`)}>⌖ 지도에서 보기</a><Link className="outline-action" href={`/species/${record.speciesId}`}>▦ 내 도감에서 보기</Link><Link className="outline-action" href={`/discoveries/view?id=${encodeURIComponent(record.id)}`}>발견 기록 상세 보기</Link><button className="outline-action" onClick={() => window.alert("커뮤니티 공유는 마일스톤 6에서 연결됩니다.")}>커뮤니티에 공유하기</button><Link className="outline-action" href="/identify">다른 생물 촬영하기</Link><Link className="text-button" href="/">홈으로 이동</Link></div>
