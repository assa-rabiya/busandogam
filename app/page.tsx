...
import { locations, recentDiscoveries } from "./data/mock-data";
...
import { getPopularDiscoverySpecies, getUnifiedDiscoveries } from "./services/discovery-summary";
...
export default function Home() {
  ...
  const unifiedRecords = getUnifiedDiscoveries(records);
  const discoveredSpeciesCount = new Set(unifiedRecords.map((record) => record.speciesId)).size;
  const discoveryCount = unifiedRecords.length;
  const visitedLocationCount = new Set(unifiedRecords.map((record) => record.locationName)).size;
  const addedPoints = records.reduce((sum, record) => sum + record.scoreAwarded, 0);
  const popularSpecies = getPopularDiscoverySpecies(unifiedRecords);
  return <AppShell>
    ...
    <section className="home-section"><div className="section-heading"><div><p className="eyebrow">MY ACTIVITY</p><h2>내 활동 요약</h2></div><Link className="text-link" href="/profile">프로필 보기 →</Link></div><div className="activity-grid">{[[discoveredSpeciesCount.toString(), "발견한 생물", "종"], [discoveryCount.toString(), "발견 기록", "회"], [visitedLocationCount.toString(), "방문 지역", "곳"], [(user.points + addedPoints).toLocaleString(), "보유 포인트", "P"]].map(([value, label, unit]) => <article key={label}><strong>{value}<small>{unit}</small></strong><span>{label}</span></article>)}</div></section>
    <section className="home-section"><div className="section-heading"><div><p className="eyebrow">TODAY&apos;S SPOTS</p><h2>오늘의 추천 탐험지</h2></div><a className="text-link" href={withBasePath("/map")}>지도에서 보기 →</a></div><div className="spot-grid">{locations.map((spot) => <article className="spot-card" key={spot.id}><ImageTile label={spot.imageLabel} alt={spot.name} imageUrl={spot.imageUrl} /><div><span className={`difficulty ${spot.difficulty}`}>● {spot.difficulty}</span><h3>{spot.name}</h3><p>{spot.speciesHint}</p><dl><div><dt>추천 시간</dt><dd>{spot.bestTime}</dd></div><div><dt>환경</dt><dd>{spot.habitat}</dd></div></dl><a href={withBasePath(`/map?place=${encodeURIComponent(spot.id)}`)} className="card-link">상세보기 →</a></div></article>)}</div></section>
    ...
    <section className="home-section popularity"><div className="section-heading"><div><p className="eyebrow">DISCOVERY TREND</p><h2>이번 달 인기 생물</h2></div></div><ol>{popularSpecies.map((item) => <li key={item.rank}><b>{item.rank}</b><strong>{item.name}</strong><span>{item.count}회 발견</span><em className="same">기록 기준</em></li>)}</ol></section>
