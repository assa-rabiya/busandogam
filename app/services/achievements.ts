...
export function getAchievements(records: Discovery[]): Achievement[] {
  const species = new Set(records.map((record) => record.speciesId)).size;
  ...
  const points = records.reduce((sum, record) => sum + record.scoreAwarded, 0);
  return [
...
export function getLeaderboard(records: Discovery[], currentName: string): LeaderboardEntry[] {
  const current = { name: currentName, points: records.reduce((sum, record) => sum + record.scoreAwarded, 0), speciesCount: new Set(records.map((record) => record.speciesId)).size, isCurrentUser: true };
