...
export function SelectedImagePreview({ image, large = false }: { image: SelectedImage; large?: boolean }) {
  ...
  const pngDemoIds = new Set(["night-crab", "sea-hare", "sea-cucumber", "blue-mussel"]);
  const source = image.previewUrl ?? (image.demoId ? withBasePath(`/images/species/${image.demoId}.${pngDemoIds.has(image.demoId) ? "png" : "webp"}`) : null);
