...
export function SelectedImagePreview({ image, large = false }: { image: SelectedImage; large?: boolean }) {
  ...
  const canRenderImage = image.kind === "upload" && image.previewUrl && !failed;
  return <div className={`selected-image-preview tone-${image.imageTone} ${large ? "large" : ""}`} role="img" aria-label={`${image.fileName} 미리보기`}>
    {canRenderImage ? <img src={image.previewUrl ?? ""} alt={image.fileName} onError={() => setFailed(true)} /> : <div className="demo-image-art"><span>{image.imageLabel}</span><i /><i /><i /></div>}
