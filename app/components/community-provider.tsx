...
type CommunityContextValue = { posts: CommunityPost[]; isReady: boolean; visitorKey: string; createPost: (draft: PostDraft, author: string) => void; toggleLike: (postId: string) => void; addComment: (postId: string, author: string, content: string) => void; deletePost: (postId: string) => void; };
...
export function CommunityProvider({ children }: { children: React.ReactNode }) {
  ...
  const createPost = useCallback((draft: PostDraft, author: string) => { const authorKey = getAuthorKey(); if (isRemoteCommunityEnabled) { void communityRepository.createPost(draft, author, authorKey).then(reloadRemote).catch(() => undefined); return; } const post: CommunityPost = { ...draft, id: globalThis.crypto?.randomUUID?.() ?? `community-${Date.now()}`, authorKey, author, createdAt: new Date().toISOString(), likes: [], comments: [] }; persist([post, ...posts]); }, [persist, posts, reloadRemote]);
  const toggleLike = useCallback((postId: string) => { const authorKey = visitorKey || getAuthorKey(); if (isRemoteCommunityEnabled) { const post = posts.find((item) => item.id === postId); if (post) void communityRepository.toggleLike(postId, authorKey, post.likes.includes(authorKey)).then(reloadRemote).catch(() => undefined); return; } persist(posts.map((post) => post.id !== postId ? post : { ...post, likes: post.likes.includes(authorKey) ? post.likes.filter((id) => id !== authorKey) : [...post.likes, authorKey] })); }, [persist, posts, reloadRemote, visitorKey]);
  const addComment = useCallback((postId: string, author: string, content: string) => { const cleaned = content.trim(); if (!cleaned) return; if (isRemoteCommunityEnabled) { void communityRepository.addComment(postId, author, cleaned).then(reloadRemote).catch(() => undefined); return; } persist(posts.map((post) => post.id !== postId ? post : { ...post, comments: [...post.comments, { id: globalThis.crypto?.randomUUID?.() ?? `comment-${Date.now()}`, author, content: cleaned, createdAt: new Date().toISOString() }] })); }, [persist, posts, reloadRemote]);
  const deletePost = useCallback((postId: string) => { const authorKey = visitorKey || getAuthorKey(); const post = posts.find((item) => item.id === postId); if (!post || post.authorKey !== authorKey) return; if (isRemoteCommunityEnabled) { void communityRepository.deletePost(postId, authorKey).then(reloadRemote).catch(() => undefined); return; } persist(posts.filter((item) => item.id !== postId)); }, [persist, posts, reloadRemote, visitorKey]);
  const value = useMemo(() => ({ posts, isReady, visitorKey, createPost, toggleLike, addComment, deletePost }), [addComment, createPost, deletePost, isReady, posts, toggleLike, visitorKey]);
