...
export function CommunityProvider({ children }: { children: React.ReactNode }) {
  ...
  const addComment = useCallback((postId: string, author: string, content: string) => { const cleaned = content.trim(); if (!cleaned) return; persist(posts.map((post) => post.id !== postId ? post : { ...post, comments: [...post.comments, { id: globalThis.crypto?.randomUUID?.() ?? `comment-${Date.now()}`, author, content: cleaned, createdAt: new Date().toISOString() }] })); if (isRemoteCommunityEnabled) void communityRepository.addComment(postId, author, cleaned).then(reloadRemote).catch(() => undefined); }, [persist, posts, reloadRemote]);
