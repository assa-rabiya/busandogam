...
type PostDraft = Omit<CommunityPost, "id" | "author" | "createdAt" | "likes" | "comments">;
type CommunityContextValue = { posts: CommunityPost[]; isReady: boolean; visitorKey: string; createPost: (draft: PostDraft, author: string) => void; toggleLike: (postId: string) => void; addComment: (postId: string, author: string, content: string) => void; };
...
export function CommunityProvider({ children }: { children: React.ReactNode }) {
  ...
  const createPost = useCallback((draft: PostDraft, author: string) => { if (isRemoteCommunityEnabled) { void communityRepository.createPost(draft, author, getAuthorKey()).then(reloadRemote).catch(() => undefined); return; } const post: CommunityPost = { ...draft, id: globalThis.crypto?.randomUUID?.() ?? `community-${Date.now()}`, author, createdAt: new Date().toISOString(), likes: [], comments: [] }; persist([post, ...posts]); }, [persist, posts, reloadRemote]);
  ...
  const value = useMemo(() => ({ posts, isReady, visitorKey, createPost, toggleLike, addComment }), [addComment, createPost, isReady, posts, toggleLike, visitorKey]);
