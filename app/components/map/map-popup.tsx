...
export function MapPopup({ group, selected, onPick, onClose }: { group: MapMarkerGroup; selected: MapDiscovery; onPick: (record: MapDiscovery) => void; onClose: () => void }) {
  ...
  return <aside className="map-popup" aria-live="polite">
    ...
    <div className="map-popup-actions"><a className="primary-action" href={withBasePath(`/discoveries/view?id=${encodeURIComponent(selected.id)}`)}>발견 상세</a><a className="outline-action" href={withBasePath(`/species?id=${encodeURIComponent(selected.speciesId)}`)}>생물 정보</a></div>
  </aside>;
}
