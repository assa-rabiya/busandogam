...
const invasiveSpeciesIds = new Set(["blue-mussel", "barnacle"]);
type OriginFilter = "전체" | "토종" | "외래종";
...
export default function CollectionPage() {
  ...
  const [origin, setOrigin] = useState<OriginFilter>("전체");
  ...
  const items = useMemo(() => speciesCatalog.map((species) => {
    ...
    .filter((item) => origin === "전체" || (origin === "외래종" ? invasiveSpeciesIds.has(item.species.id) : !invasiveSpeciesIds.has(item.species.id)))
    ...
    .sort((a, b) => sort === "발견 횟수순" ? b.count - a.count
      ...
      : b.last.localeCompare(a.last)), [category, entries, origin, sort, status, usesDemoBaseline]);
  ...
  return <ProtectedPage><AppShell>
    ...
    <section className="collection-controls"><div className="filter-tabs">{(["전체", "발견 완료", "미발견", "희귀 생물", "독성 생물"] as StatusFilter[]).map((item) => <button key={item} className={status === item ? "active" : ""} onClick={() => setStatus(item)}>{item}</button>)}</div><div className="filter-tabs origin-tabs" aria-label="토종과 외래종 구분">{(["전체", "토종", "외래종"] as OriginFilter[]).map((item) => <button key={item} className={origin === item ? "active" : ""} onClick={() => setOrigin(item)}>{item}</button>)}</div><div><select aria-label="카테고리 필터" value={category} onChange={(event) => setCategory(event.target.value)}>{["전체", "어류", "연체동물", "갑각류", "극피동물", "자포동물", "해조류", "기타"].map((item) => <option key={item}>{item}</option>)}</select><select aria-label="도감 정렬" value={sort} onChange={(event) => setSort(event.target.value as SortType)}>{(["최근 발견순", "발견 횟수순", "희귀도순", "이름순"] as SortType[]).map((item) => <option key={item}>{item}</option>)}</select></div></section>
